    ...
    <>
             ...
             {isAdmin ? (
               <div className="flex items-center gap-2">
                 <button 
                   ...
                   className="w-10 h-10 rounded-full bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500 hover:text-white flex items-center justify-center border border-emerald-500/20 transition-all"
                   title="Edit Profile"
                 >
                 ...
                 <button 
                   onClick={onLogout} 
                   className="w-10 h-10 rounded-full bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white flex items-center justify-center border border-red-500/20 transition-all"
                   title="Sign Out"
                 >
      ...
      <div className={`fixed inset-0 z-[110] bg-[#050505] transition-all duration-700 flex flex-col ${mobileMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
         ...
         <div className="flex-1 flex flex-col items-center justify-center gap-10">
            ...
            {isAdmin && (
              <button onClick={onLogout} className="text-[10px] font-black uppercase tracking-[0.5em] text-red-500 hover:text-red-400 transition-colors">Terminate Session</button>
            )}
         </div>
      </div>
    </>
